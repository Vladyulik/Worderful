...
Networker.prototype._getPayload = function() {
  if (this._hasEnough(this._payloadLength)) {
    const received = this._readBytes(this._payloadLength);
    this.socket.emit('served', received);
    this._state = 'HEADER';
  }
};

Networker.prototype._onData = function() {
  while (this._process) {
    switch (this._state) {
    case 'HEADER':
      this._getHeader();
...
Networker.prototype._send = function() {
  const contentLength = Buffer.allocUnsafe(2);
  contentLength.writeUInt16BE(this._packet.header.length);
  const combinedBuffer = Buffer.concat([contentLength, this._packet.message]);
